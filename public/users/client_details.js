...
window.addEventListener("resize", () => {
  ...
  if (allDataRows.length) { colOffset = 0; renderTable(); }
});
...
function getPctForCol(row, colIndex) {
  if (pctColIndices.has(colIndex + 1)) {
    ...
    if (raw) { const v = parseFloat(raw.replace(/[^0-9.\-]/g, "")); return isNaN(v) ? null : v; }
  }
  return null;
}
...
function calcSummary(colObj) {
  ...
  let income = 0, explotacion = 0, bankInterest = 0, pureExpense = 0; // ← pureExpense add karo
...
html += fdCard("💸", "Expense", fmt(s.expense), "Pure Expenses (4–13)", expType);
...
function renderTable() {
  ...
  if (rangeLabel) rangeLabel.textContent = totalCols > 0 ? `Columns ${colOffset + 1}–${colEnd} of ${totalCols}` : "";
...
function renderPagination() {
  ...
  for (let p = sp; p <= ep; p++) btns += `<button class="page-btn ${p === currentPage ? "active" : ""}" onclick="goToPage(${p})">${p}</button>`;
...
function openFullscreenTable() {
  ...
  if (fsRowCnt) fsRowCnt.textContent = `${dataRowCount} rows · ${headers.length} columns`;
  ...
  if (!chatInitialized) {
    if (allDataRows.length === 0) {
      const waitAndInit = setInterval(() => {
        if (allDataRows.length > 0) { clearInterval(waitAndInit); addWelcomeMessage(); chatInitialized = true; }
      }, 200);
...
function jsCalculate(intent) {
  ...
  if (type === "single") {
    ...
    return { period: col.label, income: s.income, explotacion: s.expense, bankInterest: s.bankInterest, finalResult: s.finalResult, profit: s.profit, loss: s.loss };
  }
  if (type === "compare") {
    ...
    const s1 = calcSummary(col1); const s2 = calcSummary(col2);
    function pct(a, b) { if (a === 0) return null; return parseFloat((((b - a) / Math.abs(a)) * 100).toFixed(2)); }
    return { metric: intent.metric, period1: { label: col1.label, income: s1.income, explotacion: s1.expense, finalResult: s1.finalResult, profit: s1.profit, loss: s1.loss }, period2: { label: col2.label, income: s2.income, explotacion: s2.expense, finalResult: s2.finalResult, profit: s2.profit, loss: s2.loss }, difference: { income: parseFloat((s2.income - s1.income).toFixed(2)), income_pct: pct(s1.income, s2.income), explotacion: parseFloat((s2.expense - s1.expense).toFixed(2)), explotacion_pct: pct(s1.expense, s2.expense), finalResult: parseFloat((s2.finalResult - s1.finalResult).toFixed(2)), finalResult_pct: pct(s1.finalResult, s2.finalResult), direction: s2.finalResult > s1.finalResult ? "improved" : s2.finalResult < s1.finalResult ? "declined" : "unchanged" } };
  }
  if (type === "trend") {
    const pool = monthCols.length > 0 ? monthCols : yearCols;
    if (pool.length === 0) return { error: "No period columns found" };
    const points = pool.map(col => { const s = calcSummary(col); return { period: col.label, income: s.income, explotacion: s.expense, finalResult: s.finalResult, profit: s.profit, loss: s.loss }; }).filter(p => p.income !== 0 || p.finalResult !== 0);
...
function buildIntentFromQuestion(question) {
  ...
  if (foundCols.length === 0) { for (let i = 0; i < words.length - 1; i++) { const combined = words[i] + " " + words[i + 1]; const col = findColByLabel(combined); if (col) foundCols.push(col); } }
